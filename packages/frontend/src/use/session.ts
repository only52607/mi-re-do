...
export type ContactForSessionType<T extends SessionType> = 
    T extends "group" ? Group : (T extends "friend" ? Friend :  Member)

export type SessionIdentity<T extends SessionType = SessionType> = [number, T]

export function sessionIdentityEquals<T extends SessionType = SessionType>(a: SessionIdentity<T>, b: SessionIdentity<T>) {
...
export function pushEmptySession<T extends SessionType>(type: T, contact: ContactForSessionType<T>) {
    if (globalSessionList.find((session) => session.identity[0]==contact.id && session.identity[1]==type)) return
...
function eventDispatcher(event: Event) {
    ...
    const session = globalSessionList.find((box) => sessionIdentityEquals(box.identity, sessionMeta.identity))
    if (!session) {
        globalSessionList.push({
            ...sessionMeta,
            events: [event],
